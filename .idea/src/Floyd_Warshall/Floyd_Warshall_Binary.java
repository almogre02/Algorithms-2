...
public class Floyd_Warshall_Binary {
    ...
    private static void FixPath(String[][] path, int n) {
        for (int i = 0; i < path.length; i++) {
            for (int j = 0; j < path.length; j++) {
                path[i][j] += "->("+BottlesProblem.getI(j, n)+","+BottlesProblem.getJ(j, n)+")";
            }
        }
    }
    ...
    private static void PrintAllPath(boolean[][] T, String[][] path, int n) {
        for (int i = 0; i < path.length; i++) {
            for (int j = 0; j < path.length; j++) {
                System.out.print("("+BottlesProblem.getI(i, n)+","+BottlesProblem.getJ(i, n)+")->("+BottlesProblem.getI(j, n)+","+BottlesProblem.getJ(j, n)+"):\t");
                if (T[i][j] == true)
    ...
    private static void IsExist(int i1, int j1, int i2, int j2, boolean[][] T, String[][] paths, int n) {
        int i = BottlesProblem.getIndex(i1, j1, n);
        int j = BottlesProblem.getIndex(i2, j2, n);
    ...
    private static String[][] InitPath(int size, boolean[][] T, int n) {
        ...
        for (int i = 0; i < path.length; i++) {
            for (int j = 0; j < path.length; j++) {
                if (T[i][j] == true)
                    path[i][j] = "("+BottlesProblem.getI(i, n)+","+BottlesProblem.getJ(i, n)+")";
...
//		//with path
        ...
        boolean[][] T = BottlesProblem.BottlesProblem(m,n);
